...
namespace SysLibraryWeb.TagHelpers
{
    ...
    public class LoginTypeTagHelper : TagHelper
    {
        ...
        public override Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
        {
            foreach (var loginType in LoginType)
            {
                ...
                {
                    case "UserName":
                        output.Content.AppendHtml($"<option selected=\"selected/\" value=\"{loginType}\">账号</option>");
                        break;
                    case "Email":
                        output.Content.AppendHtml(GetOption(loginType, "邮箱"));
                        break;
                    case "Phone":
                        output.Content.AppendHtml(GetOption(loginType, "手机号码"));
                        break;
                    default:break;
                }
            }
